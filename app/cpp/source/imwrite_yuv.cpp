...
jint
//JNICALL
//Java_org_codeaurora_snapcam_filter_ClearSightImageProcessor_convertAndSaveRAW10Native(
Java_com_android_camera_imageprocessor_PostProcessor_convertAndSaveRawSensorNative(
        JNIEnv *env,
        jobject obj /* this  */,
        jbyteArray rawBuffer,
        jfloatArray maskBuffer,
        jstring jSavePath) {
...
//    fout.close();
    int black_level = 63;
    float white_level_const = 255.0f / (1023.0f - black_level);

    for (int i=0; i< imH * imW; i++) {
        int pixelBLCorrected = (rawBuffer_data[i] - black_level);

        if (pixelBLCorrected < 0) {
	        result_buffer[i] = 0;
	      continue;
	    }
	    if (pixelBLCorrected > 0 && pixelBLCorrected < 8 ) {
            result_buffer[i] = pixelBLCorrected;
            continue;
        }
	    float corrected = pixelBLCorrected * mask_data[i] * white_level_const;
//        float corrected = pixelBLCorrected * white_level_const;
	    float clipped = std::min(corrected, 255.0f);
//        result_buffer[i] = uint8_t(clipped);
//
	    result_buffer[i] = lut[uint8_t(clipped)];

    }

    cv::Mat res = cv::Mat(imH, imW, CV_8U); //, &yBuffer_data[0])
    memcpy(res.data, result_buffer, imH*imW);
    cv::Mat res_cropped = cv::Mat();
    cv::Rect roi;
    roi.x = 16;
    roi.y = 8;
    roi.width = 4000;
    roi.height = 3000;
    res_cropped = res(roi);
    cv::imwrite(filename_c_str, res_cropped);
